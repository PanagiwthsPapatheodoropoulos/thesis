...
public interface EmployeeRepository extends JpaRepository<Employee, UUID> {
        ...
        /**
         * Retrieves all distinct, non-null employee positions for a given company.
         * Used to populate the position filter dropdown dynamically.
         *
         * @param companyId The company UUID.
         * @return Sorted list of distinct position strings.
         */
        @Query("SELECT DISTINCT e.position FROM Employee e JOIN e.user u " +
                        "WHERE u.company.id = :companyId AND e.position IS NOT NULL AND e.position <> '' " +
                        "ORDER BY e.position ASC")
        List<String> findDistinctPositionsByCompanyId(@Param("companyId") UUID companyId);
