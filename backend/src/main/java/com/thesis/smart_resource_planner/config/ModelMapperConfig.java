    ...
    public ModelMapper modelMapper() {
        ...
        modelMapper.typeMap(Employee.class, EmployeeDTO.class).addMappings(mapper -> {
            ...
            if (source.getUser() != null) {
                ...
                if (source.getUser().getStatus() != null) {
                    dest.setStatus(source.getUser().getStatus().name());
                }
            }
        ...
        modelMapper.typeMap(Task.class, TaskDTO.class).addMappings(mapper -> {
            ...
            mapper.skip(TaskDTO::setCompletedById);
            mapper.skip(TaskDTO::setCompletedByName);
            mapper.skip(TaskDTO::setPendingById);
            mapper.skip(TaskDTO::setPendingByName);
        }).setPostConverter(context -> {
            ...
            if (source.getCompletedBy() != null) {
                dest.setCompletedById(source.getCompletedBy().getId());
                dest.setCompletedByName(source.getCompletedBy().getUsername());
            }

            if (source.getPendingBy() != null) {
                dest.setPendingById(source.getPendingBy().getId());
                dest.setPendingByName(source.getPendingBy().getUsername());
            }

            if (source.getTags() != null) {
                dest.setTags(new LinkedHashSet<>(source.getTags()));
            }
