...
public class EmployeeService {
    ...
    public List<EmployeeWorkloadDTO> getEmployeeWorkload(UUID userId) {
        ...
        // Fetch ALL company tasks ONCE, outside the loop
        ...
        return employees.stream().map(emp -> {
            ...
            // Filter relevant tasks for THIS employee using the pre-fetched list
            List<Task> relevantTasks = allCompanyTasks.stream()
                    .filter(task -> {
                        ...
                        // Do NOT include Team or Public tasks in personal workload
                        return false;
                    })
                    .toList();
    ...
    public List<EmployeeDTO> getAllEmployees(UUID userId) {
        ...
        return employees.stream()
                ...
                .map(employee -> {
                    ...
                    // Hide salary info for non-privileged users
                    if (!canViewSalary) {
    ...
    @Caching(evict = {
            ...
            @CacheEvict(value = "employees", allEntries = true),
            @CacheEvict(value = "employeeWorkload", allEntries = true),
            @CacheEvict(value = "departments", allEntries = true) // evict so employee counts update
    })
    @Transactional
    ...
    private EmployeeDTO mapEmployeeToDTO(Employee employee) {
        ...
        if (employee.getUser() != null && employee.getUser().getStatus() != null) {
            dto.setStatus(employee.getUser().getStatus().name());
        }
    ...
    /**
     * Returns all distinct, non-empty employee positions for a given company,
     * sorted alphabetically. Used to populate the position filter dropdown
     * dynamically so no positions are hardcoded on the frontend.
     *
     * @param userId UUID of the requesting user (used to resolve company)
     * @return sorted list of distinct position strings
     */
    @Transactional(readOnly = true)
    public List<String> getDistinctPositions(UUID userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("User not found"));
        UUID companyId = user.getCompany().getId();
        return employeeRepository.findDistinctPositionsByCompanyId(companyId);
    }

    /**
     * Calculates workload distribution per department.
     */
    @Transactional(readOnly = true)
    public List<DepartmentWorkloadDTO> getDepartmentWorkloads(UUID userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ResourceNotFoundException("User not found"));
        UUID companyId = user.getCompany().getId();
        ...
        // 2. Fetch all employees and accepted assignments
        List<Employee> employees = employeeRepository.findByCompanyIdWithTeam(companyId).stream()
                .filter(emp -> emp.getUser() != null && emp.getUser().getRole() == UserRole.EMPLOYEE)
                .toList();
        ...
        // 3. Group employees by Department string
        Map<String, List<Employee>> deptEmployees = employees.stream()
                .collect(Collectors.groupingBy(
                        e -> e.getDepartment() != null && !e.getDepartment().trim().isEmpty() ? e.getDepartment()
                                : "Unassigned"));

        // 4. Calculate total public task hours
        double totalPublicHours = allCompanyTasks.stream()
                .filter(t -> t.getTeam() == null && t.getAssignedEmployeeId() == null)
                .mapToDouble(t -> {
                    double est = t.getEstimatedHours() != null ? t.getEstimatedHours().doubleValue() : 4.0;
                    return est <= 8.0 ? est : 8.0 + ((est - 8.0) * 0.25);
                })
                .sum();

        // Total company capacity for proportional public task distribution
        int totalCompanyCapacity = employees.stream()
                .mapToInt(e -> e.getMaxWeeklyHours() > 0 ? e.getMaxWeeklyHours() : 40)
                .sum();

        List<DepartmentWorkloadDTO> departmentWorkloads = new ArrayList<>();

        for (Map.Entry<String, List<Employee>> entry : deptEmployees.entrySet()) {
            String deptName = entry.getKey();
            List<Employee> deptEmps = entry.getValue();

            int deptCapacity = deptEmps.stream()
                    .mapToInt(e -> e.getMaxWeeklyHours() > 0 ? e.getMaxWeeklyHours() : 40)
                    .sum();

            // Calculate proportional public hours
            double deptPublicHours = 0;
            if (totalCompanyCapacity > 0) {
                deptPublicHours = totalPublicHours * ((double) deptCapacity / totalCompanyCapacity);
            }

            // Find all teams associated with this department
            Set<UUID> deptTeamIds = deptEmps.stream()
                    .map(e -> e.getUser().getTeam() != null ? e.getUser().getTeam().getId() : null)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toSet());

            // Department Task Hours (Assigned to department's teams, but not to a specific
            // user)
            double deptTaskHours = allCompanyTasks.stream()
                    .filter(t -> t.getTeam() != null && deptTeamIds.contains(t.getTeam().getId())
                            && t.getAssignedEmployeeId() == null)
                    .mapToDouble(t -> {
                        double est = t.getEstimatedHours() != null ? t.getEstimatedHours().doubleValue() : 4.0;
                        return est <= 8.0 ? est : 8.0 + ((est - 8.0) * 0.25);
                    })
            ...
            // Employee Assigned Hours (Specifically assigned to employees in this
            // department)
            Set<UUID> deptEmpIds = deptEmps.stream().map(Employee::getId).collect(Collectors.toSet());
            double employeeTaskHours = allCompanyTasks.stream()
                    .filter(t -> {
                        if (t.getAssignedEmployeeId() != null && deptEmpIds.contains(t.getAssignedEmployeeId()))
                            return true;
                        for (Employee e : deptEmps) {
                            if (acceptedTasksByEmployee.getOrDefault(e.getId(), Collections.emptySet())
                                    .contains(t.getId())) {
                                return true;
                            }
                        }
            ...
            double usedHours = deptPublicHours + deptTaskHours + employeeTaskHours;
            double workloadPct = deptCapacity > 0 ? (usedHours / deptCapacity) * 100.0 : 0;

            departmentWorkloads.add(DepartmentWorkloadDTO.builder()
                    .departmentName(deptName)
                    .totalCapacityHours(deptCapacity)
                    .publicTaskHours(deptPublicHours)
                    .departmentTaskHours(deptTaskHours)
                    .employeeAssignedHours(employeeTaskHours)
                    .usedHours(usedHours)
