...
public class BrevoEmailService {
    ...
    /**
     * Sends an email notification to the company owner when the join code is regenerated.
     */
    @Async
    public void sendJoinCodeRegeneratedEmail(String toEmail, String adminUsername, String companyName, String newJoinCode) {
        if (!StringUtils.hasText(apiKey)) {
            log.warn("Brevo API key missing; skipping join code regenerated email.");
            return;
        }

        String safeCompanyName = companyName != null ? companyName : "your company";
        String subject = "🔑 Security Update: Company Join Code Regenerated – " + safeCompanyName;
        
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        String formattedDate = LocalDateTime.now().format(formatter);

        String htmlContent = "<div style=\"font-family:Arial,sans-serif;max-width:600px;margin:auto;line-height:1.6\">"
                + "<div style=\"background:linear-gradient(135deg,#e11d48,#f43f5e);padding:24px;border-radius:12px 12px 0 0;text-align:center\">"
                + "<h2 style=\"color:white;margin:0\">🔑 Security Update: Join Code Changed</h2>"
                + "</div>"
                + "<div style=\"background:#ffffff;padding:28px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb\">"
                + "<p style=\"font-size:16px\">Hi <strong>" + adminUsername + "</strong>,</p>"
                + "<p>This is a security notification to confirm that the join code for your company <strong>" + safeCompanyName + "</strong> was successfully regenerated.</p>"
                + "<div style=\"background:#fffbeb;border-left:4px solid #f59e0b;padding:16px;border-radius:4px;margin:20px 0\">"
                + "<p style=\"margin:0;color:#92400e;font-weight:bold\">🕒 Change Details:</p>"
                + "<p style=\"margin:8px 0 0 0;color:#92400e;font-size:14px\">"
                + "<strong>Date & Time:</strong> " + formattedDate + "<br/>"
                + "<strong>Actioned By:</strong> " + adminUsername + "</p>"
                + "</div>"
                + "<p>The old join code has been invalidated and will no longer accept new users. Your new join code is:</p>"
                + "<div style=\"background:#f3f4f6;border:2px dashed #e11d48;border-radius:8px;padding:20px;text-align:center;margin:20px 0\">"
                + "<p style=\"font-size:28px;font-weight:bold;letter-spacing:6px;color:#e11d48;margin:0\">"
                + newJoinCode + "</p>"
                + "</div>"
                + "<div style=\"background:#fee2e2;border-left:4px solid #dc2626;padding:16px;border-radius:4px;margin-bottom:24px\">"
                + "<p style=\"margin:0;color:#991b1b;font-weight:bold\">🔒 Keep this code secure!</p>"
                + "<p style=\"margin:8px 0 0 0;color:#b91c1c;font-size:14px\">"
                + "Anyone with this new join code can request to join your company workspace. Share it only with trusted team members.</p>"
                + "</div>"
                + "<p style=\"margin-top:24px\">Best regards,<br/><strong>Smart Allocation Team</strong></p>"
                + "</div></div>";
