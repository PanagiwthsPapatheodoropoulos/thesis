...
public class ChatController {
    ...
    public ResponseEntity<ChatMessageDTO> editMessage(
            ...
            @AuthenticationPrincipal UserPrincipal currentUser) {
        ChatMessageDTO updated = chatService.editMessage(id, updateDTO.getMessage(), currentUser.getId());
    ...
    public ResponseEntity<Void> deleteMessage(
            @PathVariable UUID id,
            @AuthenticationPrincipal UserPrincipal currentUser) {
        chatService.deleteMessage(id, currentUser.getId());
    ...
    public ResponseEntity<ChatRoomDTO> createGroupChat(
            ...
            @AuthenticationPrincipal UserPrincipal currentUser) {
        ChatRoomDTO chatRoom = chatService.createGroupChat(dto, currentUser.getId());
        return ResponseEntity.status(HttpStatus.CREATED).body(chatRoom);
    }

    @PatchMapping("/group/{roomId}/rename")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'EMPLOYEE')")
    public ResponseEntity<ChatRoomDTO> renameGroupChat(
            @PathVariable UUID roomId,
            @Valid @RequestBody ChatRoomRenameDTO dto,
            @AuthenticationPrincipal UserPrincipal currentUser) {
        ChatRoomDTO chatRoom = chatService.renameGroupChat(roomId, dto, currentUser.getId());
    ...
    public ResponseEntity<Void> leaveGroupChat(
            @PathVariable UUID roomId,
            @AuthenticationPrincipal UserPrincipal currentUser) {
        chatService.leaveGroupChat(roomId, currentUser.getId());
        return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<Void> deleteGroupChat(
            @PathVariable UUID roomId,
            @AuthenticationPrincipal UserPrincipal currentUser) {
        chatService.deleteGroupChat(roomId, currentUser.getId());
        return ResponseEntity.noContent().build();
    }

    @GetMapping("/group")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'EMPLOYEE')")
    public ResponseEntity<List<ChatRoomDTO>> getGroupChats(
            @AuthenticationPrincipal UserPrincipal currentUser) {
        List<ChatRoomDTO> chats = chatService.getGroupChats(currentUser.getId());
        return ResponseEntity.ok(chats);
    }

    @GetMapping("/group/{roomId}/messages")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'EMPLOYEE')")
    public ResponseEntity<List<ChatMessageDTO>> getGroupChatMessages(
            @PathVariable UUID roomId,
            @AuthenticationPrincipal UserPrincipal currentUser) {
        List<ChatMessageDTO> messages = chatService.getGroupChatMessages(roomId, currentUser.getId());
        return ResponseEntity.ok(messages);
    }
}