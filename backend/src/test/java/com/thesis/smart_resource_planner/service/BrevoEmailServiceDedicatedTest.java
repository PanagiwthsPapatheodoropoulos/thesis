...
class BrevoEmailServiceDedicatedTest {
    ...
    @Test
    @DisplayName("Should send join code regenerated email successfully")
    void testSendJoinCodeRegeneratedEmail_Success() {
        ResponseEntity<String> response = new ResponseEntity<>("Success", HttpStatus.OK);
        when(restTemplate.exchange(anyString(), any(HttpMethod.class), any(HttpEntity.class), eq(String.class)))
                .thenReturn(response);

        emailService.sendJoinCodeRegeneratedEmail("admin@example.com", "adminUser", "Acme Inc", "NEWCODE123");
        ...
        Map<String, Object> body = entity.getBody();
        assertNotNull(body);
        assertTrue(((String) body.get("subject")).contains("Company Join Code Regenerated"));
    }
