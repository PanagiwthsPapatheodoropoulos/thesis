...
class UserServiceDedicatedTest {
    ...
    void updateUser_setsTeamAndActive() {
        ...
        dto.setStatus(com.thesis.smart_resource_planner.model.enums.UserStatus.OFFLINE);
        ...
        assertFalse(Boolean.TRUE.equals(user.getStatus() == com.thesis.smart_resource_planner.model.enums.UserStatus.ONLINE));
    }
    ...
    @Test
    @DisplayName("updateUserStatus updates user status and broadcasts")
    void updateUserStatus_success() {
        User user = new User();
        user.setId(userId);
        user.setStatus(com.thesis.smart_resource_planner.model.enums.UserStatus.OFFLINE);

        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        
        userService.updateUserStatus(userId, com.thesis.smart_resource_planner.model.enums.UserStatus.ONLINE);

        assertEquals(com.thesis.smart_resource_planner.model.enums.UserStatus.ONLINE, user.getStatus());
        verify(userRepository).save(user);
        verify(webSocketBroadcastService).broadcastUserPresence(userId, com.thesis.smart_resource_planner.model.enums.UserStatus.ONLINE);
    }
}
