...
class EmployeeServiceDedicatedTest {
    ...
    @Test
    @DisplayName("getDistinctPositions returns sorted position list for company")
    void getDistinctPositions_returnsPositionList() {
        User user = new User();
        user.setId(userId);
        user.setCompany(company);

        List<String> expectedPositions = List.of("Backend Developer", "Product Manager", "UX Designer");

        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        when(employeeRepository.findDistinctPositionsByCompanyId(company.getId()))
                .thenReturn(expectedPositions);

        List<String> result = employeeService.getDistinctPositions(userId);

        assertEquals(3, result.size());
        assertEquals("Backend Developer", result.get(0));
        verify(employeeRepository).findDistinctPositionsByCompanyId(company.getId());
    }

    @Test
    @DisplayName("getDistinctPositions throws when user not found")
    void getDistinctPositions_userNotFound_throws() {
        when(userRepository.findById(userId)).thenReturn(Optional.empty());
        assertThrows(ResourceNotFoundException.class,
                () -> employeeService.getDistinctPositions(userId));
    }
}
