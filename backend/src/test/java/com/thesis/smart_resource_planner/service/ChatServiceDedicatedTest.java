...
class ChatServiceDedicatedTest {
    ...
    @Mock
    private com.thesis.smart_resource_planner.repository.ChatRoomRepository chatRoomRepository;
    @Mock
    ...
    void getAvailableContacts_employeePaths() {
        ...
        assertTrue(contacts.stream().anyMatch(c -> "deptpeer".equals(c.getUsername())));
    }

    @Test
    @DisplayName("editMessage updates message content and sets isEdited")
    void editMessage_success() {
        ...
        when(modelMapper.map(any(), eq(ChatMessageDTO.class))).thenReturn(new ChatMessageDTO());

        chatService.editMessage(messageId, "new message", senderId);

        assertTrue(message.getIsEdited());
        assertEquals("new message", message.getMessage());
        verify(chatMessageRepository).saveAndFlush(message);
    }

    @Test
    @DisplayName("editMessage throws if not sender")
    void editMessage_unauthorized() {
        UUID messageId = UUID.randomUUID();
        ...
        assertThrows(BadRequestException.class, () -> chatService.editMessage(messageId, "new", senderId));
    }
    ...
    @Test
    @DisplayName("deleteGroupChat successfully deletes when user is creator")
    void deleteGroupChat_success() {
        UUID roomId = UUID.randomUUID();
        com.thesis.smart_resource_planner.model.entity.ChatRoom room = new com.thesis.smart_resource_planner.model.entity.ChatRoom();
        room.setId(roomId);
        room.setCreator(sender);

        when(chatRoomRepository.findById(roomId)).thenReturn(Optional.of(room));

        chatService.deleteGroupChat(roomId, senderId);

        verify(chatRoomRepository).delete(room);
    }

    @Test
    @DisplayName("deleteGroupChat throws when user is not creator or admin")
    void deleteGroupChat_throwsWhenNotCreator() {
        UUID roomId = UUID.randomUUID();
        com.thesis.smart_resource_planner.model.entity.ChatRoom room = new com.thesis.smart_resource_planner.model.entity.ChatRoom();
        room.setId(roomId);
        User otherCreator = new User();
        otherCreator.setId(UUID.randomUUID());
        room.setCreator(otherCreator);
        room.setMembers(new java.util.ArrayList<>());

        when(chatRoomRepository.findById(roomId)).thenReturn(Optional.of(room));

        assertThrows(BadRequestException.class, () -> chatService.deleteGroupChat(roomId, senderId));
