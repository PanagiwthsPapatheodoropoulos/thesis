...
class EmployeeControllerDedicatedTest {
    ...
    void setUp() {
        ...
        user.setStatus(com.thesis.smart_resource_planner.model.enums.UserStatus.ONLINE);
    ...
    @Test
    @DisplayName("getDistinctPositions delegates to service and returns list of strings")
    void getDistinctPositions_delegatesToService_returnsPositionList() {
        List<String> mockPositions = List.of("Backend Developer", "Product Manager", "UX Designer");
        when(employeeService.getDistinctPositions(ownerUserId)).thenReturn(mockPositions);

        ResponseEntity<List<String>> response = employeeController.getDistinctPositions(employeePrincipal);

        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertNotNull(response.getBody());
        assertEquals(3, response.getBody().size());
        assertTrue(response.getBody().contains("Backend Developer"));
        verify(employeeService).getDistinctPositions(ownerUserId);
    }
}
