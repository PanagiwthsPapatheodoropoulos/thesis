...
class CompanyControllerDedicatedTest {
    ...
    void testRegenerateJoinCode_Success() throws Exception {
        ...
        when(companyService.getCompanyById(adminUserId)).thenReturn(testCompanyDTO);
        doNothing().when(brevoEmailService).sendJoinCodeRegeneratedEmail(anyString(), anyString(), anyString(), anyString());
        ...
        verify(brevoEmailService, times(1)).sendJoinCodeRegeneratedEmail(
