...
class BackendClient:
    ...
    async def _request(
        ...
        **kwargs
        ...
        try:
            # Execute the asynchronous HTTP request using the shared client
    ...
    async def get_employee_by_id(
        ...
        Returns:
            Optional[Dict[str, Any]]: Employee profile dictionary or None.
        """
        try:
            headers = {
                "Authorization": f"Bearer {token}",
            ...
            logger.error(f"Failed to fetch employee by ID {employee_id}: {e}")
            return None
