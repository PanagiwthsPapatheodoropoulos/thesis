...
async def extract_skills_from_task(
    ...
    try:
        ...
        # Fetch skills from backend dynamically
        db_skills = None
        if token and x_company_id:
            try:
                db_skills = await backend_client.get_skills(x_company_id, token)
            except Exception as e:
                logger.error(f"Failed to fetch dynamic skills from backend: {e}")
...
async def batch_extract_skills(
    ...
    try:
        ...
        # Fetch skills from backend dynamically once for the batch
        token = authorization.replace("Bearer ", "")
        db_skills = None
        if token and x_company_id:
            try:
