        ...
        for category, skills in self.skill_db.items():
            ...
            for skill in skills['exact']:
                ...
                if len(skill) == 1 or skill in skills.get('word_boundary_required', []):
                    ...
                    # Match if either the exact lowercase string matches OR the normalized strings match
                    normalized_skill = re.sub(r'[\s\-\.\/_]+', '', skill_lower)
                    if skill_lower not in combined_text and normalized_skill not in normalized_text:
                        continue
            ...
            for fuzzy_term in skills['fuzzy']:
                ...
                normalized_fuzzy = re.sub(r'[\s\-\.\/_]+', '', fuzzy_lower)
                if fuzzy_lower in combined_text or normalized_fuzzy in normalized_text:
                    related_count = sum(
                        1 for term in skills['fuzzy'] 
                        if term.lower() in combined_text or re.sub(r'[\s\-\.\/_]+', '', term.lower()) in normalized_text
                    )
