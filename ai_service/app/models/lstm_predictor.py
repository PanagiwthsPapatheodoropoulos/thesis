...
class HybridDurationPredictor:
    ...
    def predict(
        ...
        try:
            ...
            if self.rf_model is None and self.gb_model is None and self.lstm_model is None:
                return self._fallback_prediction(task_features)
            ...
            # Apply safety guard for low skill proficiency if employee is assigned
            if task_features.get('has_employee'):
                avg_proficiency = float(task_features.get('avg_skill_proficiency', 0.5))
                if avg_proficiency < 0.6:
                    # Calculate baseline prediction without employee to use as baseline
                    baseline_pred = self._fallback_prediction({**task_features, 'has_employee': False})[0]
                    # Apply penalty: level 1 (0.2) -> 1.3x baseline, level 2 (0.4) -> 1.1x baseline, level 0 (0.0) -> 1.5x baseline
                    penalty_factor = 1.5 - avg_proficiency * 1.0
                    min_allowed_pred = baseline_pred * penalty_factor
                    if final_pred < min_allowed_pred:
                        final_pred = min_allowed_pred
    ...
    def _fallback_prediction(self, task_features: Dict) -> Tuple[float, float, float]:
        ...
        # Apply employee skills and experience factors if available
        if task_features.get('has_employee'):
            avg_proficiency = float(task_features.get('avg_skill_proficiency', 0.5))
            employee_exp = float(task_features.get('employee_experience', 0.0))
            
            # If proficiency is below level 3 (0.6), experience factor is 1.0 (no reduction)
            # and a penalty is applied to increase the time.
            if avg_proficiency < 0.6:
                proficiency_factor = 1.5 - avg_proficiency * 1.0  # level 1 (0.2) -> 1.3, level 2 (0.4) -> 1.1
                experience_factor = 1.0
            else:
                # Proficiency factor: level 5 (1.0) decreases duration by 50%, level 3 (0.6) decreases it by 10%
                proficiency_factor = 1.5 - avg_proficiency * 1.0
                
                # Experience benefit is scaled by skill proficiency.
                # E.g. 10 years of experience gives max 40% reduction, but only if they have high proficiency.
                experience_benefit = min(0.4, employee_exp * 0.05)
                effective_benefit = experience_benefit * avg_proficiency
                experience_factor = 1.0 - effective_benefit
            
            predicted = predicted * proficiency_factor * experience_factor
    ...
    def _safe_load_pickle(self, file_path: Path):
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    logger.error(f"Failed to load pickle {file_path} after 5 attempts: {e}")
                    raise
                time.sleep(0.05 * (2 ** attempt) + random.uniform(0, 0.05))

    def _atomic_save_pickle(self, obj, file_path: Path):
        ...
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    logger.error(f"Failed to save pickle {file_path} after 5 attempts: {e}")
                    raise
                time.sleep(0.05 * (2 ** attempt) + random.uniform(0, 0.05))
            finally:
                if temp_path.exists():
    ...
    def _safe_load_json(self, file_path: Path):
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    logger.error(f"Failed to load JSON {file_path} after 5 attempts: {e}")
                    raise
                time.sleep(0.05 * (2 ** attempt) + random.uniform(0, 0.05))

    def _atomic_save_json(self, obj, file_path: Path):
        temp_path = file_path.with_suffix(file_path.suffix + ".tmp")
        for attempt in range(5):
            try:
                ...
                if attempt == 4:
                    logger.error(f"Failed to save JSON {file_path} after 5 attempts: {e}")
                    raise
                time.sleep(0.05 * (2 ** attempt) + random.uniform(0, 0.05))
            finally:
                if temp_path.exists():
    ...
    def _save_models(self):
        """Serialized all active models to persistent storage atomically with retry fallback."""
        
        # Save LSTM model if available
        if self.lstm_model:
            lstm_path = self.model_dir / "lstm_model.h5"
            temp_path = self.model_dir / "lstm_model.h5.tmp"
            for attempt in range(5):
                try:
                    self.lstm_model.save(str(temp_path))
                    os.replace(temp_path, lstm_path)
                    break
                except (PermissionError, OSError) as e:
                    if attempt == 4:
                        logger.error(f"Failed to save LSTM model after 5 attempts: {e}")
                        raise
                    time.sleep(0.05 * (2 ** attempt) + random.uniform(0, 0.05))
                finally:
                    if temp_path.exists():
        ...
        if self.rf_model:
            self._atomic_save_pickle(self.rf_model, self.model_dir / "rf_model.pkl")
        
        # Save GradientBoosting model if available
        if self.gb_model:
            self._atomic_save_pickle(self.gb_model, self.model_dir / "gb_model.pkl")
        
        # Save feature scaler
        self._atomic_save_pickle(self.feature_scaler, self.model_dir / "feature_scaler.pkl")
        
        # Save feature names for consistency
        self._atomic_save_json(self.feature_columns, self.model_dir / "feature_columns.json")
            
    def _load_models(self):
        """Deserialized previously saved models from disk safely with retries."""
        ...
        try:
            ...
            if TENSORFLOW_AVAILABLE and lstm_path.exists():
                for attempt in range(5):
                    try:
                        ...
                        if attempt == 4:
                            logger.error(f"Failed to load LSTM model after 5 attempts: {e}")
                            raise
                        time.sleep(0.05 * (2 ** attempt) + random.uniform(0, 0.05))
            ...
            if rf_path.exists():
                self.rf_model = self._safe_load_pickle(rf_path)
            
            # Load GradientBoosting model
            gb_path = self.model_dir / "gb_model.pkl"
            if gb_path.exists():
                self.gb_model = self._safe_load_pickle(gb_path)
            
            # Load feature scaler
            scaler_path = self.model_dir / "feature_scaler.pkl"
            if scaler_path.exists():
                self.feature_scaler = self._safe_load_pickle(scaler_path)
            
            # Load feature columns for consistency check (optional)
            columns_path = self.model_dir / "feature_columns.json"
            if columns_path.exists():
                loaded_features = self._safe_load_json(columns_path)
        ...
        try:
            ...
            try:
                ...
                if samples_path.exists():
                    samples = self._safe_load_json(samples_path)
                else:
                    samples = []
            except Exception as e:
            ...
            try:
                self._atomic_save_json(samples, samples_path)
            except Exception as e:
                logger.warning(f"Could not persist recent_samples: {e}")
            
            # Periodically rebuild RF ensemble components for fast adaptation using warm_start
            if len(samples) >= 10 and X_scaled.shape[1] == self.rf_model.n_features_in_:
                ...
                # Set warm_start dynamically and increment estimator count
                self.rf_model.warm_start = True
                if self.rf_model.n_estimators < 250:
                    self.rf_model.n_estimators += 5
                
                # Fit the new trees on the recent samples
                self.rf_model.fit(X_recent, y_recent)
