...
describe("utils/api advanced mechanisms", () => {
  ...
  describe("JWT Auto-Refresh", () => {
    it("refreshes token and retries API call successfully when gets 401", async () => {
      ...
      expect(tasks).toEqual([{ id: "task-1", title: "Task 1" }]);
      ...
      // HttpOnly cookie refresh does not send refreshToken in body anymore, but if it does we'll just check the method

      const [retryUrl, retryOpts] = fetch.mock.calls[2];
      expect(retryUrl).toContain("/tasks");
      expect(retryOpts.headers.Authorization).toBeUndefined();
    });

    it("clears storage and redirects to /login if token refresh fails", async () => {
      // The refresh logic relies on cookie, so we simulate failure on the endpoint
