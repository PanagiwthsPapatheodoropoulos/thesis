...
describe("TaskDetailsModal extended coverage", () => {
  ...
  it("handles attachment upload, download, and deletion flow", async () => {
    mocks.getAttachments.mockResolvedValue([
      {
        id: "att1",
        filename: "document.pdf",
    ...
    // Test download by clicking file name link
    const filenameLink = screen.getByText("document.pdf");
    fireEvent.click(filenameLink);
    // Since download is mockResolvedValue, verify it doesn't crash and is called if mocked
    const downloadBtn = screen.getByTitle("Download file");
    fireEvent.click(downloadBtn);
    ...
    await waitFor(() => {
      expect(mocks.deleteAttachment).toHaveBeenCalledWith("att1");
    });
  });
});
