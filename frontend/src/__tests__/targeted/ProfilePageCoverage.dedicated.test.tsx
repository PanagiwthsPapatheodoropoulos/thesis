...
describe("ProfilePage coverage dedicated", () => {
  ...
  it("removes profile image when X button is clicked", async () => {
    // Provide a profile image initially
    mocks.getByUserId.mockResolvedValueOnce({
      id: "e1", userId: "u1", firstName: "John", lastName: "Doe",
      position: "Dev", department: "Engineering", profileImageUrl: "http://example.com/pic.png",
      skills: [],
    });
    render(<MemoryRouter><ProfilePage /></MemoryRouter>);
    await screen.findByAltText("Profile");

    const removeBtn = screen.getByTitle("Remove profile picture");
    fireEvent.click(removeBtn);

    // The image should be replaced by the default avatar block
    expect(screen.queryByAltText("Profile")).not.toBeInTheDocument();
    
    // The update should happen immediately upon clicking X
    await waitFor(() => expect(mocks.update).toHaveBeenCalledWith("e1", expect.objectContaining({ profileImageUrl: "" })));
  });
