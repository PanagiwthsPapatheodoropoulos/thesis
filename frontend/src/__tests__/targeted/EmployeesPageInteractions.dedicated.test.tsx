...
describe("EmployeesPage interactions", () => {
  ...
  it("opens edit flow and loads user profile for selected employee", async () => {
    ...
    await waitFor(() => expect(mocks.getAllPaginated).toHaveBeenCalled());
    const editBtn = await screen.findByTitle("Edit employee");
  ...
  it("handles department/position filters and switches tabs to Managers", async () => {
    render(
      <MemoryRouter>
        <EmployeesPage />
      </MemoryRouter>
    ...
    // Select 'Eng' from department select
    const deptSelect = document.querySelector('select[value="ALL"]');
    if (deptSelect) {
      fireEvent.change(deptSelect, { target: { value: "Eng" } });
      await waitFor(() => {
  ...
  it("handles sorting dropdown select changes", async () => {
    render(
      <MemoryRouter>
        <EmployeesPage />
      </MemoryRouter>
    ...
    // Select firstName-desc option
    const sortSelect = document.querySelector('select[value="firstName-asc"]');
    if (sortSelect) {
      fireEvent.change(sortSelect, { target: { value: "firstName-desc" } });
      await waitFor(() => {
        expect(mocks.getAllPaginated).toHaveBeenCalled();
      });
    }
