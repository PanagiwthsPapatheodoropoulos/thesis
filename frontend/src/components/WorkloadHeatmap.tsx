...
import type { WorkloadHeatmapProps, WorkloadData } from '../types';
 ...
 */
const WorkloadHeatmap: React.FC<WorkloadHeatmapProps> = ({ workloadData = [] }) => {
  const [selectedMetric, setSelectedMetric] = useState('workload');
  const { darkMode } = useTheme();

  // Group by department
  const departments: Record<string, any[]> = workloadData.reduce((acc: Record<string, any[]>, emp: any) => {
    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <select
          ...
          onChange={(e: React.ChangeEvent<any>) => setSelectedMetric(e.target.value)}
      ...
      <div className="space-y-6">
        {Object.entries(departments).map(([dept, employees]: [string, any[]]) => (
          <div key={dept}>
            <h4
