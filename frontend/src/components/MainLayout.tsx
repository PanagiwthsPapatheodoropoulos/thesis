...
import { notificationsAPI, chatAPI, employeesAPI, tasksAPI, assignmentsAPI, usersAPI } from '../utils/api';
...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
    ...
    const fetchCounts = useCallback(async () => {
        ...
        try {
            ...
            if (user.role !== 'USER') {
                promises.push(chatAPI.getUnreadCount().catch(() => 0));
            } else {
                promises.push(Promise.resolve(0));
            }

            const [notifData, chatCount] = await Promise.all(promises);
    ...
    // Listen to local notificationRead window events for instant badge sync
    useEffect(() => {
        const handleLocalNotificationRead = (e: Event) => {
            const customEvent = e as CustomEvent;
            const data = customEvent.detail;
            if (data.action === 'mark_all_read') {
                setUnreadCount(0);
                setRecentNotifications([]);
            } else if (data.action === 'mark_read' && data.notificationId) {
                setUnreadCount(prev => Math.max(0, prev - 1));
                setRecentNotifications(prev => prev.filter(n => n.id.toString() !== data.notificationId.toString()));
            }
        };

        window.addEventListener('notificationRead', handleLocalNotificationRead);
        return () => {
            window.removeEventListener('notificationRead', handleLocalNotificationRead);
        };
    }, []);
    ...
    useEffect(() => {
        ...
        const unsubs = [
            subscribe(EVENT_TYPES.NEW_NOTIFICATION, (notification: any) => {                
                ...
                <div className="flex items-center justify-between px-4 h-16">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-white">Smart Allocation Resource Planner</h1>
                                {user?.companyName && (
                        ...
                        <div className="relative">
                            ...
                            >
                                <div className="relative">
                                    ...
                                    <span className={`absolute bottom-0 right-0 w-3 h-3 border-2 border-indigo-600 rounded-full ${getStatusColor(user?.status)}`}></span>
                                </div>
                            ...
                            {showUserMenu && (
                                <>
                                    ...
                                    <div className="px-4 py-2">
                                        <p className="text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wider">Set Status</p>
                                        <div className="flex justify-between items-center bg-gray-50 dark:bg-gray-700/50 p-1.5 rounded-lg border border-gray-100 dark:border-gray-600/50">
                                            <button onClick={(e) => handleStatusChange('ONLINE', e)} className={`p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition ${user?.status === 'ONLINE' ? 'bg-gray-200 dark:bg-gray-600 shadow-sm' : ''}`} title="Online"><div className="w-3 h-3 rounded-full bg-green-500"></div></button>
                                            <button onClick={(e) => handleStatusChange('AWAY', e)} className={`p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition ${user?.status === 'AWAY' ? 'bg-gray-200 dark:bg-gray-600 shadow-sm' : ''}`} title="Away"><div className="w-3 h-3 rounded-full bg-yellow-500"></div></button>
                                            <button onClick={(e) => handleStatusChange('DO_NOT_DISTURB', e)} className={`p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition ${user?.status === 'DO_NOT_DISTURB' ? 'bg-gray-200 dark:bg-gray-600 shadow-sm' : ''}`} title="Do Not Disturb"><div className="w-3 h-3 rounded-full bg-red-500"></div></button>
                                            <button onClick={(e) => handleStatusChange('OFFLINE', e)} className={`p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition ${user?.status === 'OFFLINE' ? 'bg-gray-200 dark:bg-gray-600 shadow-sm' : ''}`} title="Appear Offline"><div className="w-3 h-3 rounded-full bg-gray-400"></div></button>
                                        </div>
                                    </div>
