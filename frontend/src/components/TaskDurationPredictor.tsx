...
const TaskDurationPredictorComponent: React.FC<TaskDurationPredictorProps & { description?: string }> = ({ taskData, onPredictionReceived, darkMode, description }) => {
  ...
  const fetchPrediction = async () => {
    ...
    try {
      ...
      const assignedEmployeeId = taskData.assignedEmployeeId || 'none';
      const tempTaskId = `preview-${taskData.priority}-${complexityScore}-${requiredSkillIds.length}-${assignedEmployeeId}`;
