...
const TaskComplexityAnalyzer: React.FC<TaskComplexityAnalyzerProps> = ({ title, description, onComplexityDetected, darkMode = false }) => {
  ...
  /**
   * Returns the Tailwind color class based on the complexity score.
   * @param {number} score - Complexity score between 0 and 1.
  ...
  /**
   * Returns the Tailwind background color class based on the complexity score.
   * @param {number} score - Complexity score between 0 and 1.
   * @returns {string} Tailwind background color class.
   */
  const getComplexityBgColor = (score: number): string => {
  ...
  /**
   * Returns the Tailwind color class based on the risk level string.
   * @param {string} riskLevel - Risk level string (HIGH, MEDIUM, or other).
   * @returns {string} Tailwind color class.
   */
      ...
      {analysis && !analyzing && !error && (
        <div className="space-y-3">
          ...
          <div className={`rounded-lg p-3 border ${
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                <TrendingUp className={`w-4 h-4 ${getComplexityColor(analysis.complexity_score)}`} />
                <span className={`font-bold text-lg ${getComplexityColor(analysis.complexity_score)}`}>
            ...
            {/* Progress Bar */}
              ...
              <div 
                className={`h-2 rounded-full transition-all duration-500 ${getComplexityBgColor(analysis.complexity_score)}`}
          ...
          <div className={`grid grid-cols-2 gap-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            ...
            <div className={`p-2 rounded ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Risk Level</span>
          ...
          {analysis.blocking_factors && analysis.blocking_factors.length > 0 && (
            <div className={`text-xs p-3 rounded border ${
              ...
              <ul className="list-disc list-inside space-y-1">
                {analysis.blocking_factors.map((factor: string, idx: number) => (
