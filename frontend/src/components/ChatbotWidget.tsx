...
const ChatbotWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const runCheck = async () => {
      ...
      if (isOpen) {
        checkInterval = setInterval(runCheck, isHealthy ? 60000 : 30000);
      }
    };
   ...
   */
  const onVoiceResult = (event: any, silenceTimeoutRef: React.MutableRefObject<any>) => {
  ...
  const onVoiceError = (event: any, silenceTimeoutRef: React.MutableRefObject<any>, autoStopTimeout: any) => {
    if (silenceTimeoutRef.current) clearTimeout(silenceTimeoutRef.current);
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event: any) => onVoiceResult(event, silenceTimeoutRef);
    recognition.onerror = (event: any) => onVoiceError(event, silenceTimeoutRef, autoStopTimeout);
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-900/50">
              {messages.map((msg, idx) => (
                <div key={`${msg.type}-${msg.timestamp.getTime()}-${msg.text.slice(0, 20)}`} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
