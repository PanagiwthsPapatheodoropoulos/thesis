...
const Pagination: React.FC<PaginationProps> = ({ currentPage, totalPages, totalElements, size, onPageChange, darkMode }) => {
  ...
  /**
   * Returns the CSS class string for a navigation button based on its disabled state and dark mode.
   * @param {boolean} disabled - Whether the button is disabled.
   * @returns {string} Tailwind CSS class string.
   */
  const getNavButtonClass = (disabled: boolean): string => {
    const base = 'p-2 rounded-lg transition';
    if (disabled) {
      return `${base} ${darkMode ? 'text-gray-600 cursor-not-allowed' : 'text-gray-400 cursor-not-allowed'}`;
    }
    return `${base} ${darkMode ? 'text-gray-400 hover:bg-gray-700 hover:text-gray-200' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`;
  };
    ...
    <div className={`flex items-center justify-between px-6 py-3 border-t mt-auto ${
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={getNavButtonClass(currentPage === 0)}
        >
        ...
        <button
          ...
          className={getNavButtonClass(currentPage >= totalPages - 1)}
        >
