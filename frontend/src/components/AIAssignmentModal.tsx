...
import type { AIAssignmentModalProps, AISuggestion } from '../types';
...
interface CandidateCardProps {
  ...
  index: number;
  darkMode: boolean;
  handleAssign: (employeeId: string, fitScore: number, confidenceScore: number) => void;
}

const CandidateCard: React.FC<CandidateCardProps> = ({ suggestion, index, darkMode, handleAssign }) => {
  let cardStyle = '';
  if (index === 0) {
    cardStyle = darkMode ? 'border-yellow-600 bg-yellow-900/30' : 'border-yellow-300 bg-yellow-50';
  } else {
    cardStyle = darkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-white';
  }

  let assignBtnStyle = '';
  if (index === 0) {
    assignBtnStyle = 'bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:shadow-lg';
  } else {
    assignBtnStyle = darkMode ? 'bg-gray-700 text-gray-200 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200';
  }

  let workloadColor = 'text-red-600';
  if (suggestion.workloadPercentage !== null) {
    if (suggestion.workloadPercentage < 50) {
      workloadColor = 'text-green-600';
    } else if (suggestion.workloadPercentage < 85) {
      workloadColor = 'text-yellow-600';
    } else if (suggestion.workloadPercentage <= 100) {
      workloadColor = 'text-orange-600';
    }
  }

  let availabilityColor = 'text-red-600';
  if (suggestion.availableHours !== null) {
    if (suggestion.availableHours > 20) {
      availabilityColor = 'text-green-600';
    } else if (suggestion.availableHours > 10) {
      availabilityColor = 'text-yellow-600';
    }
  }

  return (
    <div className={`p-5 rounded-lg border-2 transition hover:shadow-lg ${cardStyle}`}>
      <div className="flex items-start justify-between mb-3">
          ...
          <div>
            ...
            {suggestion.workloadPercentage !== null && (
              <div className={`mt-3 flex items-center gap-2 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                ...
                <span className={`font-semibold ${workloadColor}`}>
        ...
        <button
          ...
          className={`px-6 py-2 rounded-lg font-semibold transition ${assignBtnStyle}`}
        >
          Assign
        </button>
      </div>
      ...
      <div className={`rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto ${
        ...
        <div className={`px-6 py-4 border-b ${
          ...
          <div className={`flex gap-4 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            ...
            {task?.complexityScore !== undefined && task?.complexityScore !== null && (
