 ...
 */
const processTaskData = (tasks: any[], days: number) => {
  const now = new Date();
  const startDate = new Date(now.getTime() - (days * 24 * 60 * 60 * 1000));
  
  // Create week buckets
  const weeks = Math.ceil(days / 7);
  const weekData: Record<string, { completed: number; inProgress: number; pending: number }> = {};
  ...
  tasks.forEach(task => {
    ...
    const weekIndex = Math.floor((relevantDate.getTime() - startDate.getTime()) / (7 * 24 * 60 * 60 * 1000));
  ...
  const chartData = Object.entries(weekData).map(([week, data]: [string, any]) => ({
    week,
 ...
 */
const EmployeeProductivityChart: React.FC<EmployeeProductivityChartProps> = ({ employeeId, darkMode }) => {
  ...
  if (loading) {
    return (
      <div data-testid="productivity-chart" className={`p-6 rounded-lg border ${
          ...
          {['30', '60', '90'].map(range => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1 rounded-md text-sm transition ${buttonStyle}`}
              >
                {range}d
              </button>
            );
