  ...
  const fetchFreshSkills = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/employees/${employeeId}/skills`,
        {
  ...
  const handleAddSkill = async () => {
    ...
    try {
        if (employeeId) {
            ...
            let skillData;
            try {                
                const skillResponse = await fetch(`${API_BASE}/skills`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${"cookie-based"}`,
                ...
                if (skillResponse.ok) {
                    ...
                    const existingSkillResponse = await fetch(
                        `${API_BASE}/skills/name/${encodeURIComponent(newSkill.skillName)}`,
                        {
                            credentials: 'same-origin'
                        }
                    );
            ...
            const addSkillResponse = await fetch(`${API_BASE}/employees/${employeeId}/skills`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    try {
        if (employeeId && skillToRemove.id && !skillToRemove.id.toString().startsWith('temp-')) {            
            const deleteResponse = await fetch(
                `${API_BASE}/employees/${employeeId}/skills/${skillToRemove.skillId || skillToRemove.id}`,
                {
                    method: 'DELETE',
                    credentials: 'same-origin'
                }
