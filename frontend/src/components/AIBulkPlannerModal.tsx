...
interface AssignmentPlanRowProps {
  item: PlannedAssignment;
  darkMode: boolean;
  panelClass: string;
}

const AssignmentPlanRow: React.FC<AssignmentPlanRowProps> = ({ item, darkMode, panelClass }) => {
  return (
...
const mapRawBulkAssignments = (assignments: RawBulkAssignment[]): PlannedAssignment[] => {
...
const AIBulkPlannerModal: React.FC<AIBulkPlannerModalProps> = ({ isOpen, tasks, onClose, onApplied }) => {
  ...
  const fetchPlan = async () => {
    ...
    try {
      ...
      const mapped = mapRawBulkAssignments(assignments);
      setPlan(mapped);
    } catch (err: unknown) {
