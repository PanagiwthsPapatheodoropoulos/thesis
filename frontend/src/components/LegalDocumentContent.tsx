...
const LegalDocumentContent: React.FC<LegalDocumentContentProps> = ({
  ...
  let iconBoxClass = '';
  if (isTerms) {
    iconBoxClass = darkMode ? 'bg-indigo-900/40 text-indigo-300' : 'bg-indigo-100 text-indigo-700';
  } else {
    iconBoxClass = darkMode ? 'bg-emerald-900/40 text-emerald-300' : 'bg-emerald-100 text-emerald-700';
  }
