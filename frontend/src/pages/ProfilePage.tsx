...
import { User, Mail, Briefcase, Calendar, Upload, Save, RefreshCw,Building2,Award,TrendingUp,X } from 'lucide-react';
import { usersAPI, employeesAPI, API_BASE } from '../utils/api';
...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (usernameChanged) {
        try {
          const response = await fetch(`${API_BASE}/users/${user.id}/username`, {
            method: 'PATCH',
            headers: {
      ...
      login(updatedUser, "cookie-based");
              ...
              {imagePreview ? (
                <>
                  ...
                  <button
                    ...
                    className="absolute top-0 right-0 bg-red-500 text-white p-1.5 rounded-full hover:bg-red-600 transition shadow-sm z-10"
