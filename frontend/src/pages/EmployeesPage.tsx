...
const EmployeesPage = () => {
  ...
  /**
   * Fetches all distinct employee positions for this company from the backend.
   * Used to dynamically populate the position filter dropdown.
   */
  const fetchPositions = async () => {
    try {
      const data = await employeesAPI.getPositions();
      setPositions(data.filter(pos => 
        pos && 
        pos.trim() !== '' && 
        pos.toLowerCase() !== 'manager' && 
        pos.toLowerCase() !== 'administrator' && 
        pos.toLowerCase() !== 'admin' && 
        pos.toLowerCase() !== 'super admin'
      ));
    } catch (error: any) {
      setPositions([]);
    }
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await employeesAPI.update(selectedEmployee.id, {
        ...
        position: editFormData.position || selectedEmployee.position,
      ...
      if (editFormData.role !== originalUser.role) {
        ...
        const tabWillChange = (editFormData.role === 'MANAGER' && activeTab !== 'MANAGER') ||
                              (editFormData.role === 'EMPLOYEE' && activeTab !== 'EMPLOYEE');
      ...
      fetchPositions(); // Refresh positions in case a new one was added
    } catch (error: any) {
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEditFormData({ ...editFormData, position: e.target.value })}
                ...
                <p className={`text-xs mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  Type a new position or choose from existing ones
                </p>
              </div>
