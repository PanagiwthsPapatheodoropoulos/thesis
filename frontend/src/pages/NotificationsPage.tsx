...
const NotificationsPage = () => {
  ...
  const handleMarkAsRead = async (id) => {
    ...
    try {      
      ...
      // Dispatch local event to notify MainLayout immediately
      window.dispatchEvent(new CustomEvent('notificationRead', { detail: { action: 'mark_read', notificationId: id } }));
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {      
      ...
      // Dispatch local event to notify MainLayout immediately
      window.dispatchEvent(new CustomEvent('notificationRead', { detail: { action: 'mark_all_read' } }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={unreadCount > 0 ? `${unreadCount} unread` : 'All read'}
        darkMode={darkMode}
        action={
