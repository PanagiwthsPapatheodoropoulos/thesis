...
import { Plus, Users, X, UserPlus, UserMinus, AlertTriangle, ShieldAlert, Trash2, Search, User, Edit2 } from 'lucide-react';
import { teamsAPI, usersAPI, employeesAPI, API_BASE } from '../utils/api';
...
const TeamsPage = () => {
  ...
  const [editTeamFormData, setEditTeamFormData] = useState({ name: '', description: '' });
  ...
  const handleOpenEditTeam = (team) => {
    setEditTeamFormData({ name: team.name, description: team.description || '' });
  ...
  const handleSaveEditTeam = async (e) => {
    e.preventDefault();
    try {
      await teamsAPI.update(selectedTeam.id, editTeamFormData);
  ...
  const availableUsersForTeam = users.filter(u => {
    const employeeProfile = employees.find(emp => emp.userId === u.id);
    const hasEmployeeProfile = !!employeeProfile;
    ...
    const matchesSearch = !memberSearchTerm ||
      ...
      employeeProfile?.position?.toLowerCase().includes(memberSearchTerm.toLowerCase());
    return hasEmployeeProfile && notInTeam && notCurrentUser && matchesSearch;
  });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 fade-in">
          <div className={`rounded-lg p-6 w-full max-w-md max-h-[80vh] overflow-y-auto scale-in ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setShowAddMemberModal(false); setMemberSearchTerm(''); }}>
            ...
            <div className="relative mb-4">
              <input
                ...
                className={`w-full pl-10 pr-4 py-2.5 rounded-lg border focus:ring-2 focus:ring-indigo-500 outline-none transition ${
                  darkMode
                    ? 'bg-gray-800 border-gray-700 text-gray-100 placeholder-gray-500'
                    : 'bg-white border-gray-300 text-gray-900 placeholder-gray-400'
                }`}
            ...
            <div className="space-y-2">
              {availableUsersForTeam.length > 0 ? (
                availableUsersForTeam.map(availUser => {
                    ...
                    const employeeProfile = employees.find(emp => emp.userId === availUser.id);
                    return (
                  <button
                    key={availUser.id}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      {employeeProfile?.position && (
                        <span className={`text-xs px-2 py-0.5 rounded-full ${darkMode ? 'bg-indigo-900 text-indigo-200' : 'bg-indigo-100 text-indigo-700'}`}>
                          {employeeProfile.position}
                        </span>
                      )}
                    </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    {memberSearchTerm
                      ? `No employees found matching "${memberSearchTerm}"`
