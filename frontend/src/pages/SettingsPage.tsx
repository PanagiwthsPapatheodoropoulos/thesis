...
import { usersAPI, companiesAPI, blocklistAPI, API_BASE } from '../utils/api';
...
const SettingsPage = () => {
  ...
  const handleRegenerateJoinCode = async () => {
    const isConfirmed = await confirm({
      ...
      message: 'Regenerate join code? The old code will stop working immediately.',
  ...
  const handleLeaveTeam = async () => {
    ...
    setLoading(true);
    try {
      await fetch(`${API_BASE}/teams/${userTeam.id}/members/${user.id}`, {
        method: 'DELETE',
        credentials: 'same-origin'
      });
