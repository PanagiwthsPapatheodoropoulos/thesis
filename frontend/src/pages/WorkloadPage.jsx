...
const WorkloadPage = () => {
  ...
  const stats = {
    ...
    avgWorkload: workloadData.length > 0 
      ...
      : 0,
    totalActiveTasks: workloadData.reduce((sum, e) => sum + e.activeTasks, 0),
    totalCompletedTasks: workloadData.reduce((sum, e) => sum + e.completedTasks, 0)
  };

  // Chart data
  const statusDistribution = [
    { name: 'Overloaded', value: stats.overloaded, color: '#EF4444' },
    { name: 'Optimal', value: stats.optimal, color: '#10B981' },
    { name: 'Underloaded', value: stats.underloaded, color: '#F59E0B' }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-7 gap-4">
        ...
        <div className={`${containerClass} rounded-lg shadow p-4 border-l-4 border-red-500`}>
          ...
          <p className="text-3xl font-bold text-red-600">{stats.overloaded}</p>
        </div>
        ...
        <div className={`${containerClass} rounded-lg shadow p-4 border-l-4 border-yellow-500`}>
          ...
          <p className="text-3xl font-bold text-yellow-600">{stats.underloaded}</p>
        </div>
      ...
      <div className={`${containerClass} rounded-lg shadow p-6 border ${borderClass}`}>
        ...
        {(() => {
          ...
          const departmentStats = workloadData.reduce((acc, emp) => {
            ...
            acc[dept].activeTasks += emp.activeTasks;
            acc[dept].completedTasks += emp.completedTasks;
            acc[dept][emp.status.toLowerCase()]++;
            return acc;
          }, {});

          const teamData = Object.entries(departmentStats).map(([dept, stats]) => ({
            name: dept,
            avgWorkload: (stats.totalWorkload / stats.employees.length).toFixed(1),
            employees: stats.employees.length,
            activeTasks: stats.activeTasks,
            completedTasks: stats.completedTasks,
            overloaded: stats.overloaded,
            optimal: stats.optimal,
            underloaded: stats.underloaded
          }));
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex-1 min-w-[200px]">
            ...
            >
              ...
              <option value="OVERLOADED">Overloaded ({stats.overloaded})</option>
              <option value="OPTIMAL">Optimal ({stats.optimal})</option>
              <option value="UNDERLOADED">Underloaded ({stats.underloaded})</option>
            </select>
          </div>
        </div>
      </div>
