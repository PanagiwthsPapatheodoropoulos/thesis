...
import { 
  Building2, Users, Trash2, Eye, 
  TrendingUp, Shield, Search,
...
const SuperAdminDashboardPage = () => {
  ...
  const viewCompanyDetails = async (company) => {
    try {
      ...
      const [freshCompanyResponse, users, tasks, employees, departments, teams] = await Promise.all([
        fetch(`/api/super-admin/companies/${company.id}`, {
          credentials: 'same-origin',
          headers: { 
            'Content-Type': 'application/json'
          }
        }),
        fetchWithFallback(`/api/super-admin/companies/${company.id}/users`, 'Users'),
        fetchWithFallback(`/api/super-admin/companies/${company.id}/tasks`, 'Tasks'),
        fetchWithFallback(`/api/super-admin/companies/${company.id}/employees`, 'Employees'),
        fetchWithFallback(`/api/super-admin/companies/${company.id}/departments`, 'Departments'),
        fetchWithFallback(`/api/super-admin/companies/${company.id}/teams`, 'Teams')
      ]);

      let freshCompany = company;
      if (freshCompanyResponse.ok) {
        try {
          freshCompany = await freshCompanyResponse.json();
        } catch (e) {
          // fallback to list-level company data
        }
      }
  ...
  const handleDeleteCompany = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/super-admin/companies/${companyToDelete.id}`,
        {
          method: 'DELETE',
          credentials: 'same-origin'
        }
