...
const EmployeesPage = () => {
  ...
  // Modal State
  const [editEmployeeSkills, setEditEmployeeSkills] = useState([]);
  ...
  useEffect(() => {
    ...
    const unsub = subscribe(EVENT_TYPES.PROFILE_UPDATED, (data) => {
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      await employeesAPI.update(selectedEmployee.id, {
        ...
        maxWeeklyHours: parseInt(editFormData.maxWeeklyHours)
      });
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditFormData({ ...editFormData, maxWeeklyHours: e.target.value })}
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none ${darkMode
                      ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-400'
                      : 'bg-white border-gray-300 text-gray-900'
                    }`}
