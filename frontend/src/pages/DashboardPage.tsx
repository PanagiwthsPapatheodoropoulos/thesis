...
const DashboardPage = () => {
      ...
      try {
        ...
        if (isEmployee) {
          ...
          try {
            ...
            // Failed to get user team, ignore
          }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Dashboard"
        description={`Welcome back, ${user?.username}!${isEmployee ? " (Showing your team's tasks)" : ""}`}
        darkMode={darkMode}
        action={
          <button
            onClick={fetchDashboardData}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statsToShow.map((stat, i) => {
          ...
          return (
            <div 
              key={i} 
              className={`rounded-xl shadow-sm p-6 hover:shadow-md transition card-hover fade-in border ${
      ...
      {priorityBreakdown.length > 0 && (
        <div className={`rounded-xl shadow-sm p-6 border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} max-w-3xl mx-auto w-full`}>
