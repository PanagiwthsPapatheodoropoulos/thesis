...
const AIInsightsPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      // Task status changes
      subscribe(EVENT_TYPES.TASK_STATUS_CHANGED, (data) => {
        queueUpdate('TASK_STATUS_CHANGED');
      }),
      
      // Task created
      subscribe(EVENT_TYPES.TASK_CREATED, (data) => {
        queueUpdate('TASK_CREATED');
      }),
      
      // Task approved
      subscribe(EVENT_TYPES.TASK_APPROVED, (data) => {
        queueUpdate('TASK_APPROVED');
      }),
      
      // Assignment created
      subscribe(EVENT_TYPES.ASSIGNMENT_CREATED, (data) => {
        queueUpdate('ASSIGNMENT_CREATED');
      }),
      
      // Assignment accepted
      subscribe(EVENT_TYPES.ASSIGNMENT_ACCEPTED, (data) => {
              ...
              {topPerformers.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {topPerformers.slice(0, 3).map((emp, idx) => (
            ...
            <div className="space-y-6">
              {anomalies.tasks.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {anomalies.tasks.map((anomaly, idx) => (
              ...
              {anomalies.employees.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {anomalies.employees.map((anomaly, idx) => (
                      <div key={idx} className={`p-4 rounded-lg border-2 ${getSeverityColor(anomaly.severity)}`}>
                        <div className="flex items-start gap-3">
                          {getSeverityIcon(anomaly.severity)}
                          <div className="flex-1">
              ...
              {anomalies.skillGaps.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {anomalies.skillGaps.map((anomaly, idx) => (
                      <div key={idx} className={`p-4 rounded-lg border-2 ${getSeverityColor(anomaly.severity)}`}>
                        <div className="flex items-start gap-3">
                          {getSeverityIcon(anomaly.severity)}
                          <div className="flex-1">
              ...
              {anomalies.workloadImbalance.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {anomalies.workloadImbalance.map((anomaly, idx) => (
                      <div key={idx} className={`p-4 rounded-lg border-2 ${getSeverityColor(anomaly.severity)}`}>
                        <div className="flex items-start gap-3">
                          {getSeverityIcon(anomaly.severity)}
                          <div className="flex-1">
              ...
              {anomalies.complexityMismatch.length > 0 && (
                <div>
                  ...
                  <div className="space-y-3">
                    {anomalies.complexityMismatch.map((anomaly, idx) => (
                      <div key={idx} className={`p-4 rounded-lg border-2 ${getSeverityColor(anomaly.severity)}`}>
                        <div className="flex items-start gap-3">
                          {getSeverityIcon(anomaly.severity)}
                          <div className="flex-1">
