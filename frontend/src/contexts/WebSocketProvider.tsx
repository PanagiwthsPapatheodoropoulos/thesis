...
/** Connection state shape tracked by the provider. */
...
export const WebSocketProvider = ({ children }: { children: ReactNode }) => {
  ...
  const listenersRef = useRef<Map<string, Map<number, (data: any) => void>>>(new Map());
  const handlerIdCounter = useRef<number>(0);
 ...
 * @param {unknown} data - The event payload to pass to each handler.
 */
  const broadcastEvent = useCallback((eventType: string, data: any): void => {
 ...
 * @param {unknown} body - The message payload, which will be serialized to JSON.
 */
  const send = useCallback((destination: string, body: any): void => {
  ...
  const updateConnectionState = useCallback((connected: boolean, ready: boolean = false): void => {
 ...
 */
  const connectWebSocket = useCallback((): void => {
    ...
    const stompClient = new Client({
      ...
      onConnect: () => {
        ...
        // Ensure frontend state reflects online status when WebSocket connects
        ...
        if (!subs.has('profile-update')) {
          subs.set('profile-update', stompClient.subscribe(`/user/queue/profile-update`, (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
        ...
        if (!subs.has('notifications')) {
          subs.set('notifications', stompClient.subscribe(`/user/queue/notifications`, (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);              
        ...
        if (!subs.has('notification-update')) {
          subs.set('notification-update', stompClient.subscribe(`/user/queue/notification-update`, (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);
        ...
        if (!subs.has('messages')) {
          subs.set('messages', stompClient.subscribe(`/user/queue/messages`, (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);
        ...
        if (!subs.has('chat-update')) {
          subs.set('chat-update', stompClient.subscribe(`/user/queue/chat-update`, (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);
              if (data.action === 'delete') {
                broadcastEvent(EVENT_TYPES.MESSAGE_DELETED, data);
              } else if (data.action === 'mark_read' || data.action === 'mark_all_read') {
                broadcastEvent(EVENT_TYPES.MESSAGE_READ, data);
              } else if (data.id) {
                broadcastEvent(EVENT_TYPES.MESSAGE_UPDATED, data);
              }
            } catch (error) {
        ...
        if (!subs.has('task-updates')) {
          subs.set('task-updates', stompClient.subscribe(`/user/queue/task-updates`, (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);
        ...
        if (!subs.has('assignment-updates')) {
          subs.set('assignment-updates', stompClient.subscribe(`/user/queue/assignment-updates`, (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);
        ...
        if (!subs.has('global-profile-updates')) {
          subs.set('global-profile-updates', stompClient.subscribe('/topic/profile-updates', (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);
              broadcastEvent(EVENT_TYPES.PROFILE_UPDATED, data);
        ...
        if (!subs.has('global-presence-updates')) {
          subs.set('global-presence-updates', stompClient.subscribe('/topic/presence', (msg: IMessage) => {
            if (!mountedRef.current) return;
            try {
              const data = JSON.parse(msg.body);
              broadcastEvent(EVENT_TYPES.PRESENCE_UPDATED, data);
            } catch (error) {
  ...
  }, [token, user, authReady, broadcastEvent, updateConnectionState, updateUser]);
